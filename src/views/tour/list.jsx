...
const TourList = () => {
  ...
  const allTours =
    tours?.pages.filter((page) => page?.data).flatMap((page) => page.data) ||
    [];
  return (
    <>
      {isSettingLoading ? (
        ...
        >
          ...
          <div className="relative px-8 text-start">
            <h2 className="mb-4 text-4xl sm:text-5xl font-bold text-white lg:text-6xl">
              {setting?.value?.title ?? "[Judul wisata belum diatur]"}
            </h2>
          </div>
        </section>
      )}
        ...
        <div className="w-full px-6 sm:px-0 max-w-lg md:max-w-3xl lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl grid grid-cols-3 lg:grid-cols-4 gap-y-6 bg-gradient-to-b from-slate-100 via-blue-[#F3F9FB] to-blue-[#F3F9FB]">
          <div className="bg-transparent rounded-s-md col-span-4 grid grid-cols-6">
            <div className="col-span-6 grid grid-cols-6 gap-8">
              ...
              <div className="col-span-6 grid grid-cols-6 md:gap-x-4 gap-y-2 md:gap-y-4 justify-items-center">
                {isLoading ||
                ((!tours ||
                  !tours.pages[0] ||
                  tours.pages[0]?.data.length === 0) &&
                  isFetching) ? (
                  ...
                  <>
                    {allTours.map((tour) => (
                      <TourCard
                        ...
                        map={tour.link.gmap}
                        website={tour.link.website}
                        email={tour.link.email}
                        socmed={tour.link.sosmed}
                      />
                    ))}
